...
def draw(win, images,player_car, computer_car, game_info):
    ...
    time_text = MAIN_FONT.render(
        f"Tempo: {game_info.get_level_time()}s", 1, (255, 255, 255))
...
def handle_collision(player_car, computer_car, game_info):
    ...
    if computer_finish_poi_collide != None:
        blit_text_center(WIN, MAIN_FONT, "Voce Perdeu")
        pygame.display.update()
...
while run:
    ...
    while not game_info.started:
        blit_text_center(WIN, MAIN_FONT, f"Precione  tecla para começar o nivel {game_info.level}!")
        pygame.display.update()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            break
        """""
        Evento para desenhar o caminho do computer car
    ...
    if game_info.game_finished():
        blit_text_center(WIN, MAIN_FONT, "Parabens ganhaste o jogo")
        pygame.time.wait(7000)
        game_info.reset()
        player_car.reset()
        computer_car.reset()
